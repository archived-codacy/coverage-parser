  ...
  "parse" should {
    "identify report as invalid" when {
      "file does not follow the Cobertura format" in {
        ...
        val reader = CloverParser.parse(new File("."), new File("src/test/resources/test_cobertura.xml"))

        reader shouldBe 'left
      }

      "file does not exist" in {
        val reader = CloverParser.parse(new File("."), new File("src/test/resources/non-existent.xml"))
    ...
    "identify the report as valid" when {
      "report has packages" in {
        val reader = CloverParser.parse(new File("."), new File("src/test/resources/test_clover.xml"))
        reader shouldBe 'right
      }

      "report does not have packages" in {
        val readerWithoutPackages = CloverParser
          .parse(new File("/home/codacy-php/"), new File("src/test/resources/test_clover_without_packages.xml"))
    ...
    "return the same report with or without packages" in {
      ...
      val readerWithPackages =
        CloverParser.parse(new File("/home/codacy-php/"), new File("src/test/resources/test_clover.xml"))
    ...
    "return a report with the expected number of files" in {
      val report =
        CloverParser.parse(new File("/home/codacy-php/"), new File("src/test/resources/test_clover.xml")).right.value
